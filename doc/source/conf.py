...
.. |Integer_t| replace:: :math:`\dsettype{Integer}`

.. |Float_t| replace:: :math:`\dsettype{Float}`

.. |String_t| replace:: :math:`\dsettype{String}`
...
isphx_objpath = osp.join('isphx','{0}')
...
# Augment rst_epilog with autogenerated replaces for crosslinking to
#  datasets in specific specification versions

# Regex for parsing version
...
for dirname in os.listdir(specroot):
    ...
    if osp.isdir(osp.join(specroot, dirname)):
        ...
        for fname in [_ for _ in os.listdir(osp.join(specroot, dirname))
            ...
            anch_str = osp.splitext(fname)[0][1:]

            # Regex for parsing anchors
            p_anch = re.compile(
                     '^\\.\\. _spec_{0}-([A-Z0-9_]+):$'.format(anch_str))

            # Pull the version information
